...
func (r *DesignateProducerReconciler) generateServiceConfigMaps(
	...
	if len(instance.Spec.RedisHostIPs) == 0 {
		return fmt.Errorf("missing configuration. Unable to deploy designate-central without redis endpoints")
	}
	...
	templateParameters := map[string]interface{}{
		"CoordinationBackendURL": backendURL,
	}

	cms := []util.Template{
		// Custom ConfigMap
		{
			Name:          designate.ConfigVolumeName(designateproducer.Component),
			Namespace:     instance.Namespace,
		...
		{
			Name:         designate.DefaultsVolumeName(designateproducer.Component),
			Namespace:    instance.Namespace,
