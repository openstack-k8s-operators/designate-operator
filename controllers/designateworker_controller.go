...
func (r *DesignateWorkerReconciler) reconcileNormal(ctx context.Context, instance *designatev1beta1.DesignateWorker, helper *helper.Helper) (ctrl.Result, error) {
	...
	ctrlResult, err = getSecret(ctx, helper, instance.Namespace, &instance.Status.Conditions, designate.GetCommonScriptsSecretName(instance), &configMapVars, "")
	if err != nil {
		return ctrlResult, err
	}

	ctrlResult, err = getSecret(ctx, helper, instance.Namespace, &instance.Status.Conditions, designate.GetCommonConfigSecretName(instance), &configMapVars, "")
	...
	ctrlResult, err = getSecret(ctx, helper, instance.Namespace, &instance.Status.Conditions, designate.GetCommonDefaultOverwritesName(instance), &configMapVars, "")
	// note r.getSecret adds Conditions with condition.InputReadyWaitingMessage
	// when secret is not found
	if err != nil {
		return ctrlResult, err
