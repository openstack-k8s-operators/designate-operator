...
func (r *DesignateWorkerReconciler) GetLogger(ctx context.Context) logr.Logger {
	return log.FromContext(ctx).WithName("Controllers").WithName("DesignateWorker")
}
...
// the DesignateWorker object against the actual cluster state, and then
...
func (r *DesignateWorkerReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager) error {
	...
	if err := mgr.GetFieldIndexer().IndexField(context.Background(), &designatev1beta1.DesignateWorker{}, passwordSecretField, func(rawObj client.Object) []string {
		// Extract the secret name from the spec, if one is provided
		cr := rawObj.(*designatev1beta1.DesignateWorker)
		if cr.Spec.Secret == "" {
			return nil
		}
	...
	if err := mgr.GetFieldIndexer().IndexField(context.Background(), &designatev1beta1.DesignateWorker{}, caBundleSecretNameField, func(rawObj client.Object) []string {
		// Extract the secret name from the spec, if one is provided
		cr := rawObj.(*designatev1beta1.DesignateWorker)
	...
	// index topologyField
	if err := mgr.GetFieldIndexer().IndexField(context.Background(), &designatev1beta1.DesignateWorker{}, topologyField, func(rawObj client.Object) []string {
	...
	svcSecretFn := func(_ context.Context, o client.Object) []reconcile.Request {
		...
		// get all Worker CRs
		apis := &designatev1beta1.DesignateWorkerList{}
	...
	return ctrl.NewControllerManagedBy(mgr).
		For(&designatev1beta1.DesignateWorker{}).
...
func (r *DesignateWorkerReconciler) reconcileNormal(ctx context.Context, instance *designatev1beta1.DesignateWorker, helper *helper.Helper) (ctrl.Result, error) {
	Log := r.GetLogger(ctx)
	...
	ctrlResult, err = getSecret(ctx, helper, instance.Namespace, &instance.Status.Conditions, designate.ScriptsVolumeName(designate.ServiceName), &configMapVars, "")
	if err != nil {
		return ctrlResult, err
	}

	ctrlResult, err = getSecret(ctx, helper, instance.Namespace, &instance.Status.Conditions, designate.ConfigVolumeName(designate.ServiceName), &configMapVars, "")
	...
	ctrlResult, err = getSecret(ctx, helper, instance.Namespace, &instance.Status.Conditions, designate.DefaultsVolumeName(designate.ServiceName), &configMapVars, "")
	// note r.getSecret adds Conditions with condition.InputReadyWaitingMessage
	// when secret is not found
	if err != nil {
		return ctrlResult, err
...
func (r *DesignateWorkerReconciler) generateServiceConfigMaps(
	ctx context.Context,
	h *helper.Helper,
	instance *designatev1beta1.DesignateWorker,
	...
	//
	// create custom Configmap for designate-worker-specific config input
	...
	cms := []util.Template{
		{
			Name:          designate.ConfigVolumeName(designateworker.Component),
			Namespace:     instance.Namespace,
		...
		{
			Name:         designate.DefaultsVolumeName(designateworker.Component),
			Namespace:    instance.Namespace,
