...
func (r *DesignateBackendbind9Reconciler) checkPoolHasZones(
	...
	if err != nil {
		...
		if strings.Contains(err.Error(), "no running galera pod found") {
			Log.Info("Galera pod not ready yet, cannot check zones for pool removal")
			// Return error to trigger requeue - pool removal validation will be retried
			return false, fmt.Errorf("galera not ready for pool validation: %w", err)
		}
		Log.Error(err, "Failed to get pool name to ID mapping")
		return false, fmt.Errorf("failed to get pool mapping: %w", err)
	}
