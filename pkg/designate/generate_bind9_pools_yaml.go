...
func validateMultipoolConfig(config *MultipoolConfig) error {
	...
	for i, pool := range config.Pools {
		...
		if pool.BindReplicas <= 0 {
			return fmt.Errorf("pool %s has invalid bindReplicas %d (must be greater than 0)", pool.Name, pool.BindReplicas)
		}
	}

	// Ensure default pool exists and cannot be removed
	if !hasDefault {
		return fmt.Errorf("default pool '%s' must be present in multipool configuration", DefaultPoolName)
	}
