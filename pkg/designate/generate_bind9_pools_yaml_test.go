...
func TestValidateMultipoolConfig(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			wantErr: true,
			errMsg:  "default pool 'default' must be present",
		},
		{
			name: "empty pools",
			config: &MultipoolConfig{
		...
		},
		{
			...
			wantErr: true,
			errMsg:  "duplicate pool name: default",
		},
		{
			...
			wantErr: true,
			errMsg:  "invalid bindReplicas 0 (must be greater than 0)",
		},
		{
			...
			wantErr: true,
			errMsg:  "invalid bindReplicas -1 (must be greater than 0)",
		},
		{
